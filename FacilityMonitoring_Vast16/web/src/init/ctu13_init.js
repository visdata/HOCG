...
//sizeScale=new SizeScale();
    ...
    this.tStart='18:12:15:0';
    //this.tStart='1:1:0:0';
    //this.tEnd='2:23:59:59';
    //this.tEnd='17:23:59:59';
    this.tEnd='18:12:40:0';
...
//var color_map={'tcp:25':300,'tcp:6667':274,'tcp:80':100,'udp:161':150,'icmp':0};
var color_map={'tcp:6667':[[60,100,0.85],0],'udp:161':[[150,50,0],1],'icmp':[[0,50,0],2]};
        //var color_map = {'AirTmp':300, 'Heat':170, 'Coil':100, 'WaterTmp':150, 'Cool':0, 'WaterFlow':50, 'Equip':250,'Damper':350,'Human':204};
...
leftLayer.upper_axis_time_measure='minute';
...
var optionData=[
    ...
    {
        index:1,text:'Hier: ',title:'Topology layout of HOCG graph',class:'graphLayout',values:['LR','TB','Force'],init:'TB'
    },
    {
        index:2,text:'Anomaly Filter : ',title:'Anomaly Score Threshold',class:'hFilter',values:[0.3,0.35,0.4,0.45,0.5,0.55,0.60,0.65,0.7,0.75,0.8,0.85,0.9].map(function(d){return String(d)}),init:'0.4',
        input:{checked:true,position:'left',key:'h',father:'nodeType'}
    },
    {
        index:3,text:'EdgeLabel       : ',title:'Show Edge Label',class:'edgeLabel',values:['off','on'],init:'off'}
...
function changeOption(d){
    if(d.fatherID==1){
        ...
        console.log('setting layout engine in changeOption ');
        console.log(d.values[d.selectID]);
        leftLayer.layout_engine=d.values[d.selectID];
        console.log(leftLayer.layout_engine);
    }
...
$(function (){
    $(".hide").click(function(){
        if($(".left_nav dl dd ").css('display')=='none'){
            $(".left_nav dl dd ").show();
            newoptionSVG.style('pointer-events','auto')
        }

        else if($(".left_nav dl dd ").css('display')=='block'){
            $(".left_nav dl dd ").hide();
            newoptionSVG.style('pointer-events','none')
        }
   ...
   // layer.attr("transform", leftLayer.rScale);
    ...
    if(rScale){
        ...
        layer.selectAll('.label')
            .attrs({
                "y":function(d){
                    d.labelDeltaY=d.nodeR+ d.AHeight;
...
gSizeBarBrush.call(brush)
    .call(brush.move, [x(leftLayer.minNodeSize),x(leftLayer.maxNodeSize)]);
