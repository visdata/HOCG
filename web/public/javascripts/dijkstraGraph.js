...
function dijkstra(graph,source,destination){
	...
	this.previousNode={};
    this.nodeRecorder=[];
	this.dijkDistance=new Array();
	this.dijkDistance[source.name]=0;
	this.pq=new MinPQ();
	var nodes=graph.getAllNodes();
	length=nodes.length;
	for(var i=0;i<length;i++){
		if(nodes[i]!=source){
			this.dijkDistance[nodes[i].name]=Number.POSITIVE_INFINITY;
		}
        this.pq.push(nodes[i],this.dijkDistance[nodes[i].name]);
	}
	...
	while(this.pq.size()!=0){
		...
		for (var i = 0; i < adjList.length; i++) {
			v=adjList[i];
			if(this.dijkDistance[u.name]!=Number.POSITIVE_INFINITY){
				alt=this.dijkDistance[u.name]+u.weight[i];
				if(alt<this.dijkDistance[v.name]){
					this.dijkDistance[v.name]=alt;
					this.previousNode[v.name]=u.name;
                    this.nodeRecorder[v.name]= u.name;
                    this.pq.remove(v);
                    this.pq.push(v,this.dijkDistance[v.name]);
				}
			}
		}
	}
    var targetID=destination.name;
    var sourceID=source.name;
    var shortestPath=[targetID];
    var num=0
    while(this.nodeRecorder.length>0&&shortestPath[shortestPath.length-1]!=sourceID){
        if(num > 100){
            shortestPath=[];
            break;
        }

        shortestPath.push(this.previousNode[targetID]);
        targetID = this.previousNode[targetID];
        num+=1;

    }
    var len=shortestPath.length;
    for(var i=0;i<len/2;i++){
        var temp=shortestPath[i];
        shortestPath[i]=shortestPath[len-i-1];
        shortestPath[len-i-1]=temp;
    }
    var shortestEdges=[];
    for(var i=0;i<len-1;i++){
        var key=shortestPath[i]+'_'+shortestPath[i+1];
        shortestEdges.push(key);
    }
//    console.log(shortestEdges);

    if(typeof destination==='undefined'){
        return [];
	}else return shortestEdges;
...
function bellman_ford(graph,source,destination){
	...
	return this.dijkDistance[destination.name];
